...
app.get("/v1/venues", async (req, res) => {
    try {
        ...
        const venues = await getCachedOrFetch(Number(latitude), Number(longitude), venueType);
        res.json(venues);
    }
    catch (error) {
...
app.post("/v1/venues/:id/upvote", async (req, res) => {
    try {
...
app.post("/v1/venues/:id/downvote", async (req, res) => {
    try {
